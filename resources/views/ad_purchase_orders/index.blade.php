                                    ...
                                    <div class="status-action-group">
                                        @if($canUpdateAdpoStatus && in_array($order->status, $editableAdpoStatuses))
                                            <button type="button"
                                                ...
                                                data-has-proof="{{ $order->paymentProofs->isNotEmpty() || $order->proof_of_payment ? '1' : '0' }}"
                                                data-proof-url="{{ $order->proof_of_payment ? asset($order->proof_of_payment) : '' }}"
                                                data-proof-files="{{ e($order->paymentProofs->map(function ($proof) { return ['name' => $proof->original_name, 'path' => asset($proof->path), 'size' => $proof->file_size]; })->values()->toJson()) }}"
